...
import { SeriesListResultModel, SingleSeriesResultModel } from '../../api-models/seriesModels';
...
const seriesController = {
  ...
  getSeriesList: (searchterm: string, page?: number, abortSignal?: AbortSignal): Promise<SeriesListResultModel> =>
    http<SeriesListResultModel>({
      request: `${process.env.REACT_APP_OMDB_URL}?s=${searchterm}&type=series&page=${page ?? 1}&apikey=${process.env.REACT_APP_OMDB_APIKEY}`,
      init: {
        signal: abortSignal
      },
